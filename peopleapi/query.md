...
Graph based API to get People or Employee information, 
 ```javascriptfunction fancyAlert(arg) {if(arg) {$.facebox({div:"#foo"})}}``` including their relationships like line managers, reporting line and office location. You can use an Employee query if employee id is known or use a Search query to look up an Employee.
